...
namespace Hal {
	...
	struct HalUartDriver : public Hal::IUartDriver {
		...
		static void activate_IRQ(UartPort port, uint32_t preempt, uint32_t sub) {
			// Activer les IRQs
			IRQn_Type irq;
			switch (port) {
		...
		DriverStatus transmit(int8_t handleIndex, const uint8_t* data, uint16_t size, uint32_t timeout) override {
			return From_hal(HAL_UART_Transmit(&uartHandles[handleIndex], const_cast<uint8_t*>(data), size, timeout));
		}

		DriverStatus receive(int8_t handleIndex, uint8_t* data, uint16_t size, uint32_t timeout) override {
			return From_hal(HAL_UART_Receive(&uartHandles[handleIndex], data, size, timeout));
		}

		DriverStatus transmit_it(int8_t handleIndex, const uint8_t* data, uint16_t size) override {
			return From_hal(HAL_UART_Transmit_IT(&uartHandles[handleIndex], const_cast<uint8_t*>(data), size));
		}

		DriverStatus receive_it(int8_t handleIndex, uint8_t* data, uint16_t size) override {
			return From_hal(HAL_UART_Receive_IT(&uartHandles[handleIndex], data, size));
		}
