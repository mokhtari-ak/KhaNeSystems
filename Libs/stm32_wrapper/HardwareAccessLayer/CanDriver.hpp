    ...
    */
    struct HalCanDriver : public ICanDriver {
        ...
        * @Brief Stocke les handles HAL pour chaque port CAN utilisé.
		...
		static uint32_t MapRjw(CanResynchJumpWidth rjw) {
            // Mapping des valeurs statiques aux constantes HAL
            switch (rjw) {
        ...
        */ 
        ...
        void config_filter() {
            ...
            sFilterConfig.FilterBank = config::FilterBank;
            sFilterConfig.FilterMode = (config::FilterMode == CanFilterMode::Mask) ? CAN_FILTERMODE_IDMASK : CAN_FILTERMODE_IDLIST;
            sFilterConfig.FilterScale = (config::FilterScale == CanFilterScale::Scale_32bit) ? CAN_FILTERSCALE_32BIT : CAN_FILTERSCALE_16BIT;
            sFilterConfig.FilterIdHigh = config::FilterIdHigh;
            sFilterConfig.FilterIdLow = config::FilterIdLow;
            sFilterConfig.FilterMaskIdHigh = config::FilterMaskHigh;
            sFilterConfig.FilterMaskIdLow = config::FilterMaskLow;
            sFilterConfig.FilterFIFOAssignment = (config::Fifo == CanRxFifo::FIFO_0) ? CAN_RX_FIFO0 : CAN_RX_FIFO1;
            sFilterConfig.FilterActivation = config::FilterActivation ? CAN_FILTER_ENABLE : CAN_FILTER_DISABLE;
            sFilterConfig.SlaveStartFilterBank = config::SlaveStartFilterBank;
...
/**
* @Brief Ces fonctions sont les callbacks d'interruption que la HAL de ST appelle lorsqu'un message est reçu dans la FIFO 0 ou FIFO 1.
*        Elles font le lien entre les interruptions gérées par la HAL et les callbacks C++ enregistrés dans la classe HalCanDriver.
* @Note Vous devez implémenter ces fonctions dans un fichier source (ex: CanIRQHandler.cpp) et les déclarer extern "C" pour éviter le name mangling.
*/ 
extern "C" {
