...
namespace Hal {
	struct HalGpioDriver : public IGpioDriver {
		...
		/**
		 * @Brief Callbacks pour les interruptions GPIO.
		 * @Note Ces callbacks sont appelés par les ISR d'interruption GPIO (ex: EXTI0_IRQHandler) que vous devez implémenter dans un fichier source (ex: GpioIRQHandler.cpp).
		 * @Note Chaque callback correspond à une ligne d'interruption (ex: EXTI0_IRQHandler pour la ligne 0, EXTI1_IRQHandler pour la ligne 1, etc.) et doit appeler HalGpioDriver::handle(pin) avec le numéro de pin correspondant.
		 * @Note L'utilisation d'une map permet de stocker dynamiquement les callbacks pour chaque pin, ce qui est plus flexible que d'avoir des variables statiques séparées pour chaque pin. Cependant, assurez-vous de gérer correctement l'accès à cette map dans un contexte d'interruption (ex: en désactivant les interruptions lors de la modification de la map ou en utilisant des mécanismes de synchronisation appropriés).
		 * @Note Remplacer cette map par Eigen3 dans un futur proche pour réduire l'empreinte mémoire et améliorer les performances. Eigen3 offre des structures de données optimisées pour les systèmes embarqués, ce qui peut être bénéfique pour la gestion des ressources dans un environnement à contraintes.
		 * @Note Assurez-vous que les callbacks enregistrés dans cette map sont valides et gèrent correctement les opérations nécessaires lors d'une interruption GPIO (ex: lecture de l'état du pin, exécution de la logique métier associée, etc.). Vous pouvez également envisager d'ajouter une gestion d'erreur pour les cas où un callback n'est pas enregistré pour une pin donnée ou pour les cas où une interruption est déclenchée pour une pin qui n'est pas configurée pour les interruptions.
		 * @Note Enfin, n'oubliez pas de tester votre implémentation avec différents scénarios d'interruption GPIO pour vous assurer que les callbacks sont correctement appelés et que le comportement du système est conforme à vos attentes dans toutes les conditions d'utilisation.
		 */
		inline static std::map<int, std::function<void()>> callbacks = { };
		
		/**
		 * @Brief Initialise le GPIO.
		 * @Note Cette fonction initialise le GPIO en fonction de la configuration spécifiée.
		 */
		...
		template <GpioConfigPolicy T>
		GPIO_InitTypeDef getHALConfig() override{
