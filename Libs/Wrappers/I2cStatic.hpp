...
namespace Wrapper {
	...
	template<I2cConfigPolicy config, typename Driver = HalI2cDriver>
		...
		public:
			...
			DriverStatus master_transmit(uint16_t devAddress, const uint8_t* data, uint16_t size, uint32_t timeout = HAL_MAX_DELAY) {
			...
			DriverStatus master_receive(uint16_t devAddress, uint8_t* data, uint16_t size, uint32_t timeout = HAL_MAX_DELAY) {
				return m_driver.master_receive(handleIndex, devAddress, data, size, timeout);
			}

			DriverStatus mem_write(uint16_t devAddress, uint16_t memAddress, uint16_t memAddSize, const uint8_t* data, uint16_t size, uint32_t timeout = HAL_MAX_DELAY) {
				return m_driver.mem_write(handleIndex, devAddress, memAddress, memAddSize, data, size, timeout);
			}

			DriverStatus mem_read(uint16_t devAddress, uint16_t memAddress, uint16_t memAddSize, uint8_t* data, uint16_t size, uint32_t timeout = HAL_MAX_DELAY) {
			...
			DriverStatus master_transmit_it(uint16_t devAddress, const uint8_t* data, uint16_t size) {
				return m_driver.master_transmit_it(handleIndex, devAddress, data, size);
			}

			DriverStatus master_receive_it(uint16_t devAddress, uint8_t* data, uint16_t size) {
				return m_driver.master_receive_it(handleIndex, devAddress, data, size);
			}
        
			DriverStatus mem_write_it(uint16_t devAddress, uint16_t memAddress, uint16_t memAddSize, const uint8_t* data, uint16_t size) {
				return m_driver.mem_write_it(handleIndex, devAddress, memAddress, memAddSize, data, size);
			}

			DriverStatus mem_read_it(uint16_t devAddress, uint16_t memAddress, uint16_t memAddSize, uint8_t* data, uint16_t size) {
			...
			void attach_callbacks(std::function<void()> tx_cb, std::function<void()> rx_cb, std::function<void()> error_cb) {
				HalI2cDriver::activate_IRQ(config::Port, uint32_t preempt, uint32_t sub);
		...
		private:
			...
			Wrapper::GpioStatic<typename config::SclPin> m_scl_pin;
			Wrapper::GpioStatic<typename config::SdaPin> m_sda_pin;
		};
